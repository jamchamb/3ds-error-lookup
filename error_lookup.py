...
def bitmask(value, n):
    """Extract the n least significant bits from value"""
    return value & (2**n - 1)


def code_string(name, table, code):
    result = '%s: ' % (name)
    if code in table:
        result += '%s' % (table[code])
    else:
        result += '%u (not in table)' % (code)
    result += '\n'
...
class TDSError:
    ...
    def __init__(self, code):
        self.code = code
        self.description = bitmask(code, 10)
        self.module = bitmask((code >> 10), 8)
        self.unknown = bitmask((code >> 18), 3)
        self.summary = bitmask((code >> 21), 6)
        self.level = bitmask((code >> 27), 5)

    def __str__(self):
        result = code_string('Description', description_table,
                             self.description)
        result += code_string('Module', module_table, self.module)
        result += code_string('Summary', summary_table, self.summary)
        result += code_string('Level', level_table, self.level)

        if self.unknown != 0:
            result += 'Unknown value: %u' % (self.unknown)

        return result.rstrip('\n')


def main():
    parser = argparse.ArgumentParser("3DS Error Code lookup")
    parser.add_argument("code", type=str, help="32-bit error code in hex")
